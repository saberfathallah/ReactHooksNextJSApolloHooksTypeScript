...
export const updateCacheAfterDeleteComment = (cache, data, currentCategoryId): void => {
  if (currentCategoryId !== '') {
    ...
    const index = cloneData.posts.findIndex(
      (post) => post.id === data.deleteComment.postId,
    );
    const comments = cloneData.posts[index].comments.filter(
      (comment) => comment.id !== data.deleteComment.commentId,
    );
  ...
  const comments = cloneAllPosts.posts[foundIndex].comments.filter(
    (comment) => comment.id !== data.deleteComment.commentId,
  );
