...
export const updateCacheAfterUpdateComment = (cache, data, currentCategoryId) => {
  if (currentCategoryId !== '') {
    ...
    const index = cloneData.posts.findIndex(post => post.id === data.updateComment.comment.postId);
  ...
  const foundIndex = cloneAllPosts.posts.findIndex(post => post.id === data.updateComment.comment.postId);
